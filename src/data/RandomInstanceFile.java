...
public class RandomInstanceFile {
		...
		for (Job job : s.getJobs()) {
			for (Task task : job.getMaps())
				for (int it : task.getDataLocations().keySet())
					if(0 != task.getDataLocations().get(it)){
				        System.out.println("job id: "+job.getJobID()+" task id: "+task.getTaskID()+" task type: "+task.getType()+" matrixsize: "+task.getDataLocations().size()+" node id: "+it+" data size: "+task.getDataLocations().get(it));
					}
	...
	//2015/12/20 copy and revise from above
	//2015/12/20 chage curSlot->curNode
	...
	public static void main(String[] args) {
		...
		RandomInstanceFile ri = new RandomInstanceFile(p, "E:\\TestData\\");
		try {
