...
public class Schedule {
	...
	public long getLowerBound(){
		lowerBound = Math.max(getLowerBound_1(), getLowerBound_2());
		//lowerBound = getLowerBound_1();
		return lowerBound;
	}
	public long getLowerBound_1()
	{
		long max1 = 0;
		long max2 = 0;
		
		Task t = null;
		for(int i = 0; i < this.reduces.size(); i++)
		{
			t = reduces.get(i);
			t.setSortKey(t.getProcessTime() + t.getInputSize() / Cluster.LOCAL_RATE);
		}
		...
		Collections.sort(reduces, new TaskComparator());
		Collections.sort(maps, new TaskComparator());
		
		for(int j = 0; j < cluster.getReduceSlot() * cluster.getNodeNum() && j < maps.size(); j++)
			max2 += maps.get(j).getSortKey();
		for(int i = 0; i < reduces.size(); i++)
			max2 += reduces.get(i).getSortKey();
		
		for(int j = 0; j < cluster.getMapSlot() * cluster.getNodeNum() && j < reduces.size(); j++)
			max1 += reduces.get(j).getSortKey();
		for(int i = 0; i < maps.size(); i++)
			max1 += maps.get(i).getSortKey();
		
		max2 /= (cluster.getReduceSlot() * cluster.getNodeNum());
		max1 /= (cluster.getMapSlot() * cluster.getNodeNum());
		
		return Math.max(max1, max2);
	}
	public long getLowerBound_2()
	{
		long b = 0; 
		long a = 0;
		long c1 = 0;
		...
		for(int i = 0; i < reduces.size(); i++)
			a += (reduces.get(i).getProcessTime() + reduces.get(i).getInputSize() / Cluster.LOCAL_RATE);
		a /= cluster.getReduceSlot() * cluster.getNodeNum();
		
		List<Task> maxMaps = new ArrayList<Task>();
		for(int i = 0; i < jobs.size(); i++)
		{
			long _max = Long.MIN_VALUE;
			int index = 0;
			for(int j = 0; j < jobs.get(i).getMaps().size(); j++)
			{
				Task map = jobs.get(i).getMaps().get(j);
				map.setSortKey(map.getInputSize() / Cluster.LOCAL_RATE + map.getProcessTime());
				if(map.getSortKey() > _max){
					_max = (long)map.getSortKey();
					index = j;
				}
			}
			maxMaps.add(jobs.get(i).getMaps().get(index));
		}
		
		Collections.sort(maxMaps, new TaskComparator());
		b = (long)maxMaps.get(0).getSortKey();
		c1 = a + b;
		...
		for(int i = 0; i < maps.size(); i++)
			a += (maps.get(i).getProcessTime() + maps.get(i).getInputSize() / Cluster.LOCAL_RATE);
		a /= cluster.getMapSlot() * cluster.getNodeNum();
		
		List<Task> maxReduces = new ArrayList<Task>();
		for(int i = 0; i < jobs.size(); i++)
		{
			long _max = Long.MIN_VALUE;
			int index = 0;
			for(int j = 0; j < jobs.get(i).getReduces().size(); j++)
			{
				Task reduce = jobs.get(i).getReduces().get(j);
				reduce.setSortKey(reduce.getInputSize() / Cluster.LOCAL_RATE + reduce.getProcessTime());
				if(reduce.getSortKey() > _max){
					_max = (long)reduce.getSortKey();
					index = j;
				}
			}
			maxReduces.add(jobs.get(i).getReduces().get(index));
		}
		
		Collections.sort(maxReduces, new TaskComparator());
		b = (long)maxReduces.get(0).getSortKey();
		
		c2 = a + b;
		
		return Math.max(c1, c2);//a + b;
	}
	...
	//根据完成时间（已经获得完成时间）计算当前作业顺序的Total TimeCost
	...
	public List<Job> minimizePenaltyCost(List<Job> joblist){
		...
		SES.add(jlist);
		for (Job job : joblist) {
			for(int ii = 0; ii < SES.size(); ii++){
				List<Job> list = SES.get(ii);
...
//						}
	...
	//对作业中的map任务按照LPT排序
//	public void sortMapTaskWithLPT(Job job){
//		Collections.sort(job.getMaps(), new taskComparator());
//	}
	//对作业中的reduce任务按照LPT排序
//	public void sortReduceTaskWithLPT(Job job){
...
//	}
	...
	public List<Job> GetJobSequence(List<Job> joblist){
		...
		List<Job> tepl = new ArrayList<Job>();
		tepl.add(listB.get(0));
		listsA.add(tepl);
		listB.remove(0);
		boolean flag = true;
		while (!listB.isEmpty()) {
