...
import model.JobIDComparator;
import model.Schedule;
import model.SlotComparator;
import model.Task;
import model.TaskComparator;

public class LPTTaskGreedyImproved implements Method {

	private Schedule schedule;
	private Comparator slotCompare;
	private String rule;
	
	public LPTTaskGreedyImproved(Schedule s, String rule){
		this.schedule = s;
		this.rule = rule;
	...
	public void start() {
		...
		Collections.sort(schedule.getJobs(), new JobIDComparator());
		
		long cmax = Long.MIN_VALUE;
		for(int i = 0; i < schedule.getReduces().size(); i++)
		{
			task = schedule.getReduces().get(i);
			long earliest = Long.MAX_VALUE;
			int node = 0;
			long current  = 0;
			for(int k = 0; k < schedule.getCluster().getReduceSlot() * schedule.getCluster().getNodeNum(); k++)
			{
				int local = schedule.calculateSetupTime(task, schedule.getCurAvailList().get(k));
				//if(local >= 2) continue;
				if(schedule.getCurAvailList().get(k).getCurFinishTime() + task.getProcessTime() + task.getSetupTime() < earliest)
		...
		//adjust the tasks according to job's MapFinishTime order on each slot
		for(int i = 0; i < schedule.getJobs().size(); i++)
			schedule.getJobs().get(i).setFinishTime(0);
		for(int s = 0; s < schedule.getCluster().getReduceNodes().size(); s++)
		{
			DataNode slot = schedule.getCluster().getReduceNodes().get(s);
			for(int t = 0; t < slot.getTasks().size(); t++)
			{
				Task reduce = slot.getTasks().get(t);
				//if(s == 0) map.getJob().setMapFinishTime(0);
				reduce.setSortKey(schedule.getJobs().indexOf(reduce.getJob()));
			}
			Collections.sort(slot.getTasks(), new TaskComparator());
			...
			{
				Task reduce = slot.getTasks().get(t);
				reduce.setStartTime(Math.max(currentTime, reduce.getJob().getMapFinishTime()));
				reduce.setFinishTime(reduce.getStartTime() + reduce.getProcessTime() + reduce.getSetupTime());
				
				currentTime = reduce.getFinishTime();
				
				if(currentTime > reduce.getJob().getFinishTime()){
					reduce.getJob().setFinishTime(reduce.getFinishTime());
					if(reduce.getJob().getFinishTime() > cmax)
						cmax = reduce.getJob().getFinishTime();
				}
	...
	@Override
	public void output() {
		System.out.println("The makespan of the schedule under LPT task greedy heuristic improved method is: " + schedule.getMakespan());
	...
	public static void main(String[] args) {
		...
		RandomInstanceFile rf = new RandomInstanceFile(p, "G:\\Research\\Data\\MR_I_BI_200_1.txt");
		RandomInstanceFile rf1 = new RandomInstanceFile(p, "G:\\Research\\Data\\MR_I_BI_200_1.txt");
		RandomInstanceFile rf2 = new RandomInstanceFile(p, "G:\\Research\\Data\\MR_I_BI_200_1.txt");
		try {
			...
			SlotGreedyHeuristicLocal gh = new SlotGreedyHeuristicLocal(s1, "JR-LPT");
			gh.init();
			gh.start();
			gh.output();
			...
			LPTTaskGreedyLocal gh1 = new LPTTaskGreedyLocal(s2, "LPT-task");
			//SlotGreedyHeuristic gh1 = new SlotGreedyHeuristic(s2, "LPT-TASK");
			gh1.init();
			gh1.start();
			gh1.output();
			
			/*Schedule s3 = rf2.newInstance();
			//System.out.println(s1.getLowerBound_1());
			//System.out.println(s1.getLowerBound_2());
			System.out.println(s3.getLowerBound());
			LPTTaskGreedyImproved gh11 = new LPTTaskGreedyImproved(s3, "LPT-TASK");
			//SlotGreedyHeuristic gh11 = new SlotGreedyHeuristic(s2, "LPT-JOB");
			gh11.init();
			gh11.start();
			gh11.output();*/
