...
public class GreedyHeuristicReduceLocal implements Method {
	...
	public GreedyHeuristicReduceLocal(Schedule s, String rule){
		this.schedule = s;
		this.rule = rule;
	...
	public void start() {
		...
		for(int i = 0; i < schedule.getJobs().size(); i++)
		{
			...
			for(int j = 0; j < job.getMaps().size(); j++)
			{
				...
				int node = 0;
				//find max map tasks nodes
				int maxTaskNode = 0;
				int m = Integer.MIN_VALUE;
				for(int machine : job.getSlotMapTaskNum().keySet())
				{
					if(job.getSlotMapTaskNum().get(machine) > m){
						m = job.getSlotMapTaskNum().get(machine);
						maxTaskNode = machine;
					}
				}
				...
				for(int k = 0; k < schedule.getCluster().getMapSlot() * schedule.getCluster().getNodeNum(); k++)
				{
					...
					if(local >= 2) continue;
					if(maxTaskNode > 0 && schedule.getCurAvailList().get(k).getNodeID() % schedule.getCluster().getNodeNum() 
							!= maxTaskNode)
						continue;
				...
				if(job.getSlotMapTaskNum().get(key) != null)
				{
					...
					//if(oldNum > job.getMaxMapMachine())
				}else
					job.getSlotMapTaskNum().put(key, 1);
				
				
				
				//job.getMapTaskSlots().add(schedule.getCurAvailList().get(node));
				//job.getMapMachines().add(schedule.getCurAvailList().get(node).getNodeID()
						 //% schedule.getCluster().getNodeNum());
			}
		}
			...
			for(int j = 0; j < job.getReduces().size(); j++)
			{
				...
				{*/
					...
					for (int k = 0; k <= reduces.size() - 1; k++) {
						DataNode rslot = reduces.get(k);
						if(!job.getMapMachines().contains(rslot.getNodeID() % schedule.getCluster().getNodeNum()))
								continue;
...
//ArrayList<Integer> list = task.getHostList();
	...
	public static void main(String[] args) {
		...
		RandomInstanceFile rf = new RandomInstanceFile(p, "G:\\Research\\Data\\NewData\\MR_I_BI_NEW_100_1.txt");
		RandomInstanceFile rf1 = new RandomInstanceFile(p, "G:\\Research\\Data\\NewData\\MR_I_BI_NEW_100_1.txt");
		try {
			//Schedule ss = rf1.newInstance();
			...
			GreedyHeuristicReduceLocal gh1 = new GreedyHeuristicReduceLocal(s2, "JR-LPT");
			gh1.init();
			gh1.start();
			gh1.output();
