...
public class TestOmega {
	...
	public static void main(String[] args) {
		...
		try {
			...
			{
				p.setP_omega(ome);
				System.out.println("--------------------------Omega :" + ome
						+ "--------------------------------");
			for (int node : nodes) {
				System.out.println("--------------------------Omega: " + ome + " node: " + node
						+ "--------------------------------");
				p.setP_node(node);
				dataWriter = new BufferedWriter(new FileWriter(
						"G:\\Research\\Data\\Results\\NewResults\\"
								+ p.getP_node() + "_" + ome + "_Methods_Data_Omega.txt", false));
				presentWriter = new BufferedWriter(new FileWriter(
						"G:\\Research\\Data\\Results\\NewResults\\"
								+ p.getP_node() + "_" + ome + "_Methods_Present_Omega.txt", false));
				dataWriter.write("Instance\tLowerBound\tEATime\tEFTime\tLPTTime\t\tEA\tEF\tLPT\r\n");
				presentWriter.write("Instance\tEA\tEF\tLPT\t\tEATime\tEFTime\tLPTTime\r\n");
				for (int s : size) {
